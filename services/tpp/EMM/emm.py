"""
Error Management Module (EMM) for TPP Microservice
...
Follows the centralized error handling pattern established in RCM but adapted for TPP (Microservice 02).
"""
...
class ErrorManagementModule:
    ...
    def __init__(self):
        ...
        self.MICROSERVICE_CODE = "02"  # TPP
        
        # Module codes (hex values for each TPP module)
        self.MODULE_CODES = {
            "TPDM": "01",   # Text Processing Data Module
            "SWM": "02",    # Spam Word Management Module
            "FTM": "03",    # Filter Text Module
            "LPM": "04",    # Language Processing Module
        ...
        self.recovery_strategies = {
            # File and I/O errors
            "0101020E001": self._recover_file_not_found,
            "0101020E002": self._recover_permission_denied,
            "0101020E003": self._recover_disk_space_full,
            "0101020E004": self._recover_file_corrupted,
            
            # JSON and data parsing errors
            "0101020E005": self._recover_invalid_json,
            "0101020E006": self._recover_missing_required_field,
            "0101020E007": self._recover_invalid_data_type,
            "0101020E008": self._recover_encoding_error,
            
            # Network and API errors
            "0101020E009": self._recover_network_timeout,
            "0101020E00A": self._recover_connection_refused,
            "0101020E00B": self._recover_api_rate_limit,
            "0101020E00C": self._recover_authentication_failed,
            
            # Memory and resource errors
            "0101020E00D": self._recover_memory_overflow,
            "0101020E00E": self._recover_resource_exhausted,
            "0101020E00F": self._recover_thread_timeout,
            
            # Configuration and validation errors
            "0101020E010": self._recover_invalid_config,
            "0101020E011": self._recover_missing_dependency,
            "0101020E012": self._recover_invalid_argument,
            "0101020E013": self._recover_validation_failed,
            
            # Text processing specific errors
            "0101020E014": self._recover_language_detection_failed,
            "0101020E015": self._recover_spam_list_corrupted,
            "0101020E016": self._recover_text_encoding_error,
            "0101020E017": self._recover_filter_overload,
            "0101020E018": self._recover_spam_list_missing,
            "0101020E019": self._recover_text_too_large,
            "0101020E01A": self._recover_processing_timeout
        }
    ...
    def generate_error_code(self, module_name: str, class_name: str, function_name: str, sub_function: str = "001") -> str:
        ...
        Args:
            module_name: Name of the module (e.g., "TPDM")
    ...
    def log_error_with_generation(self, module_name: str, class_name: str, function_name: str, error_message: str, sub_function: str = "001", recovery_attempted: bool = False, context: Dict[str, Any] = None):
        ...
        try:
            ...
            fallback_code = "0101020E00000001"
    ...
    async def _recover_missing_required_field(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if field_name and data is not None:
                ...
                default_values = {
                    "message_text": "",
                    "request_id": f"tpp_recovered_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    "timestamp": datetime.now().isoformat(),
                    "TPP_flag": 1,
                    "language": "unknown",
                    "source_block": "TPP"
    ...
    async def _recover_encoding_error(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if file_path:
                ...
                encodings = ['utf-8', 'latin-1', 'cp1252', 'iso-8859-1', 'cp1256']  # Added Arabic encoding
    ...
    async def _recover_memory_overflow(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if hasattr(self, 'spam_word_cache'):
                self.spam_word_cache.clear()
    ...
    # TPP specific recovery strategies
    async def _recover_language_detection_failed(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        """Recovery strategy for language detection failures."""
        try:
            # Default to Persian if language detection fails
            self.logger.info("Language detection failed, defaulting to Persian")
            return True
        except Exception as e:
            self.logger.error(f"Failed to recover from language detection failure: {e}")
            return False
    
    async def _recover_spam_list_corrupted(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        """Recovery strategy for corrupted spam word lists."""
        try:
            # Try to reload default spam lists
            spam_list_path = context.get("spam_list_path") if context else None
            if spam_list_path:
                backup_path = Path(spam_list_path).with_suffix(".backup")
                if backup_path.exists():
                    shutil.copy2(backup_path, spam_list_path)
                    self.logger.info(f"Restored spam list from backup: {spam_list_path}")
                    return True
            
            self.logger.info("Spam list corruption detected - will use default filtering")
            return True
        except Exception as e:
            self.logger.error(f"Failed to recover from spam list corruption: {e}")
            return False
    
    async def _recover_text_encoding_error(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        """Recovery strategy for text encoding errors."""
        try:
            # Try to normalize text encoding
            text = context.get("text") if context else None
            if text:
                # Convert to UTF-8 if possible
                import unicodedata
                normalized = unicodedata.normalize('NFKC', text)
                self.logger.info("Normalized text encoding")
                return True
            
            return False
        except Exception as e:
            self.logger.error(f"Failed to recover from text encoding error: {e}")
            return False
    
    async def _recover_filter_overload(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        """Recovery strategy for filter overload."""
        try:
            # Reduce processing load temporarily
            await asyncio.sleep(0.5)
            self.logger.info("Applied brief delay for filter overload recovery")
            return True
        except Exception as e:
            self.logger.error(f"Failed to recover from filter overload: {e}")
            return False
    
    async def _recover_spam_list_missing(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        """Recovery strategy for missing spam word lists."""
        try:
            # Create minimal default spam list
            spam_list_path = context.get("spam_list_path", "spam_words/default.txt")
            path = Path(spam_list_path)
            path.parent.mkdir(parents=True, exist_ok=True)
            
            # Create basic spam word list
            default_spam_words = ["spam", "test", "example"]
            with open(path, 'w', encoding='utf-8') as f:
                for word in default_spam_words:
                    f.write(f"{word}\n")
            
            self.logger.info(f"Created minimal default spam list: {spam_list_path}")
            return True
        except Exception as e:
            self.logger.error(f"Failed to recover from missing spam list: {e}")
            return False
    
    async def _recover_text_too_large(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        """Recovery strategy for oversized text."""
        try:
            # Suggest text truncation or chunking
            max_length = context.get("max_length", 100000) if context else 100000
            text_length = context.get("text_length", 0) if context else 0
            
            if text_length > max_length:
                self.logger.info(f"Text too large ({text_length} chars) - suggest chunking or truncation")
                # Could implement automatic chunking here
                return True
            
            return False
        except Exception as e:
            self.logger.error(f"Failed to recover from oversized text: {e}")
            return False
    
    async def _recover_processing_timeout(self, error_message: str, context: Dict[str, Any] = None) -> bool:
        """Recovery strategy for processing timeouts."""
        try:
            # Increase timeout or implement async processing
            await asyncio.sleep(0.1)
            self.logger.info("Processing timeout recovery - suggesting async processing")
            return True
        except Exception as e:
            self.logger.error(f"Failed to recover from processing timeout: {e}")
            return False
    ...
    async def generate_error_codes_for_codebase(self, codebase_path: Path) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info("Starting automatic error code generation for TPP codebase")
            ...
            self.logger.info(f"Generated {len(new_error_codes)} new error codes for TPP")
    ...
    async def start(self):
        ...
        self.is_active = True
        self.logger.info("TPP EMM: Module started successfully")
    
    async def stop(self):
        """Stop the EMM module."""
        self.is_active = False
        self.logger.info("TPP EMM: Module stopped successfully")
...
async def log_error(service: str, module: str, function: str, error: str):
    """Legacy compatibility method for TPP error logging."""
    emm = ErrorManagementModule()
    await emm.log_error_with_generation(module, "UnknownClass", function, error)
